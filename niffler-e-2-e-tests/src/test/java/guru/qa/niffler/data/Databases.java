...
public class Databases {
    ...
    public static <T> T transaction(Function<Connection, T> function, String jdbcUrl, int transactionIsolationLevel) {
        ...
        try {
            connection = connection(jdbcUrl, transactionIsolationLevel);
    ...
    public static <T> T xaTransaction(int transactionIsolationLevel, XaFunction<T>... actions) {
        UserTransaction ut = new UserTransactionImp();
        try {
            ut.begin();
            T result = null;
            for (XaFunction<T> action : actions) {
                result = action.function.apply(connection(action.jdbcUrl, transactionIsolationLevel));
            }
            ut.commit();
            return result;
        } catch (Exception e) {
    ...
    public static void xaTransaction(int transactionIsolationLevel, XaConsumer... actions) {
        UserTransaction ut = new UserTransactionImp();
        try {
            ut.begin();
            for (XaConsumer action : actions) {
                action.function.accept(connection(action.jdbcUrl, transactionIsolationLevel));
            }
            ut.commit();
        } catch (Exception e) {
            try {
...
//                    dsBean.setDefaultIsolationLevel(transactionIsolationLevel);
    ...
    public static Connection connection(String jdbcUrl, int transactionIsolationLevel) {
